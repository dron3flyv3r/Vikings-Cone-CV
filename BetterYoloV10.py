...
class ConePosition:
    def __init__(
        ...
        x2: float,
        y2: float,
        objectType: Literal["yellow", "blue", "orange", "unknown"],
        results: Results,
        conf: float,
    ):
        self.x1 = x1
...
class Cones:
    def __init__(
        self, cones: list[ConePosition], img_width: int, img_height: int, time: float
    ):
        self.cones = cones
...
class BetterYoloV10:
    ...
    def raw_detect(self, img, conf=0.25) -> list[Results]:
        ...
        results = self.model.predict(img, conf=conf, max_det=12, save=True)

        return results

    def test_detect(self, img):
        if self.model is None:
            self.model = self._setup_model()
